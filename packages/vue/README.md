...
- 中性色阶三通道严格相等(R = G = B),整条不掺色相。**纯度守在 surface**:浅色 surface `#ffffff`、深色 surface `#000000`,bg 让开一档(`#fafafa` / `#0a0a0a`)—— 内容坐的地方才是「干净」的主战场(此处推翻过早期「页底纯白纯黑」的极性,机检同步翻转)。
...
元件的底色是 **surface**(纯白 / 纯黑 —— 纯度守在内容坐的地方,机检),页底 **bg**(canvas)让开一档(`#fafafa` / `#0a0a0a`),两者必须两色(机检)。区分主角是**精心设计的分层阴影**,发丝线只是配角勾边。soft 色一律与 surface(纯极)混色,不与 bg 混;**深色下持久 soft 填充的混色比 = 浅色 ×2**(12→24、14→28)—— 有彩色低比例混进纯黑不可读,与墨档 6→9 的上浮同理;inset 深色取 `#262626` 同因。
...
**border 与 shadow 是一体的**:surface 的视觉定义是一个总量,两者共同构成轮廓,不是两个独立装饰。由此推出**视觉接替**规则:focus 时**边框加粗,视觉上盖住原 border+shadow 那条边缘带** —— 阴影全程纹丝不动,接替是纯视觉的,粗边主导了眼睛对轮廓的读取。实现零位移且**向外长**(阴影画在外面,接替它的加厚当然朝外):**发丝线边框全程不变色**,focus 环由 **box-shadow 首槽**承担 —— `0 0 0 var(--hn-field-ring-w) var(--hn-field-ring-color)` 排在海拔阴影层前面,静止 spread 0、聚焦 2px。**环是长出来、缩回去的(spread 插值),不是淡入淡出的**:过渡只动 spread,颜色是每状态的常量(常态 accent、invalid danger),恒色相由结构保证。这条是从两个反面教训里逼出来的:其一,让边框染 accent 再退回 8% 透明黑,预乘插值会让边框中途穿过一段发黑的泥潭(实测 250ms 处 `rgba(15,52,49,.11)`,比两个端点都重)——**过渡的两个端点必须同色相,否则中间态发脏**,与「压暗填充=颜色变脏」同源;其二,改成定宽 2px 色带做纯透明度淡出,恒色相了,但出场是一圈**全尺寸残影原地褪色**,拖满整个时长,读作「出比入慢」——**状态轮廓的显隐必须是几何的,透明度淡出只配给墨与字色**。spread 收回是物理退场,同样时长下利落得多(HeroUI 同款机制)。其余错法记录在案:彩色细线叠在中性定义上(太弱)、阴影退场(surface 掉下去)、阴影换彩色(霓虹)、向内加粗(长错方向)、Tailwind `--tw-*` 阴影槽机制(inset 占位错配导致插值跳变;手写同构列表的首槽插值没有这个问题)、出场曲线用 move(中载磨蹭)。
...
**总纲(两轴模型):时长属于通道,曲线属于方向 —— 什么用什么由原则推出,不靠判断。**

轴一,时长由**通道**决定,永不随事件变:

| 通道                     | 档位                          | 例                                  |
| ------------------------ | ----------------------------- | ----------------------------------- |
| 墨(填充 / 状态层 / 字色) | `fast` 200                    | hover 落墨退墨、link 字色、按压缩放 |
| 状态轮廓(focus 环)       | `fast` 200                    | 环 spread 生长与收回,双向同档       |
| 浮层                     | `slow` 450 进 / `exit` 200 出 | Dialog / Popover                    |
| 物理体                   | 自带保真值(组件 token)        | 波纹 150/75/375、spinner 700        |

轴二,曲线由**过渡的性质**决定(修订版):**显隐类双向 `enter`** —— 不论载体是透明度(字色、墨层、遮罩)还是尺寸从零生长(focus 环 spread),从无到有和原路返回都取前载,前载才可感;`move` 只管**既存元素**在两个位置/尺寸之间的迁移(标签切换、手风琴、collapse);浮层进场 → `enter` / `enter-strong`,撤离 → `exit`;用户施力那一下 → `press`。曾把「出」一律映射到 `move`,结果状态收回中载磨蹭,时长相等也显慢——`move` 的用途本来就是迁移,不管显隐。

推论:一次事件可触多个通道,各通道按自己的档位走;但**输入件是单拍的特例**——墨、底色、边框、环同为 `fast`,聚焦这一下所有通道齐步走,不许 200/300 混拍(混拍时环总比墨晚收,整体读作拖泥带水);「出比入快」只属于浮层通道;「动画可见」是通道档位的下限约束。
...
| 场景                                         | 时长                        | 曲线                           |
| -------------------------------------------- | --------------------------- | ------------------------------ |
| hover 微反馈 / 按压 / 开关拨动 / 勾选        | `fast` 200                  | 墨 `enter` / 缩放 `press`      |
| 小浮层进场(Tooltip / Popover / Menu / Toast) | `base` 300                  | `enter`                        |
| **大面积浮层进场**(Dialog / Drawer / Sheet)  | `slow` 450                  | **`enter-strong`**             |
| 一切浮层出场                                 | `exit` 200                  | `exit`                         |
| 标签切换 / 手风琴展开 / 尺寸变化             | `base` 300                  | `move`                         |
| 列表错峰进场                                 | `base` 300                  | `enter` + `stagger` 24         |
| **focus 环显隐**(`hn-field`)                 | `fast` 200 双向             | 双向 `enter`(spread 几何生长)  |
| **选中态颜色进退**                           | `base` 300 双向             | 双向 `enter`                   |
| **确定性进度 / 加载环 / 滚动联动**           | 由数据驱动                  | **`linear`**                   |
| 波纹生长 / 浮现 / 退场                       | 150 / 75 / 375(fork 保真值) | `linear` / `move`,见「ripple」 |
| **布局重排 / 共享元素 / 拖拽**               | —                           | **走 motion-v,不用 CSS 曲线**  |

**「动画可见」原则**:状态的进入与退出都必须可感知,而可感知的门槛随载体不同——**纯颜色淡变**过短几乎不可见,必须 ≥ `base`;**几何生长自带可见性**(运动比透明度可感),focus 环 spread 在 `fast` 200 就清晰可读,不许为了凑档位把它拖到 `base`(拖了就是残影感的另一种写法)。收回闪没同样违反有进必有出。**「出比入快」只适用于浮层撤离**(元素要消失),持续元素的状态收回不适用——`hn-field` 全通道 `fast` 双向 `enter`,进出对称单拍。
...
浮层出场只有一条曲线:200ms 尺度上再分大小是无意义的档位膨胀。
...
| token                | 值    | 用途                     |
| -------------------- | ----- | ------------------------ |
| `--hn-duration-fast` | 200ms | 微反馈:hover 墨 / 按压   |
| `--hn-duration-base` | 300ms | 状态进出 / 标准进场      |
| `--hn-duration-slow` | 450ms | 大面积浮层               |
| `--hn-duration-exit` | 200ms | 浮层撤离,约进场的 0.6 倍 |

**优雅不能快**(拍板):整体档位从 120/180/240/110 上移到 200/300/450/200 —— 从容是基调的一部分,早先「超过 300ms 即拖沓」的断言作废。浮层出场仍比进场快约 0.6 倍并用反向曲线;持续元素的状态收回不适用「出快」,双向同档。
...
| 输入面:Input / Textarea / Select 触发器(`hn-field`)               | 填充落墨一步                    | focus:墨退净 + accent 环长出 | 墨落在填充里      |
...
3. 是聚焦目标而非按压目标吗(表单输入件)?是 → `hn-field`,**墨的第三种落法:落在填充里**(`<input>` 是替换元素,没有伪元素可用)。hover = fill 向 fg 混一档(同一个 `--hn-state-hover-opacity`,深色自动升档);**focus = 墨退净、fill 回本色 + accent 环从边缘长出**(见「surface 与阴影」):box-shadow 首槽的 spread 从 0 长到 2px,视觉上盖住静止态的边缘带,海拔阴影与发丝线边框全程不动;secondary 平时无边,环同样从无长出。**hover / press 不做 border 文章,border 也不参与 focus** —— outline 弃用(`outline: none`),环与海拔阴影同住 box-shadow,一个属性一拍过渡。invalid 是状态不是交互:静止时是**常规粗细**的 danger 线 + danger 淡墨(颜色标状态,定义不变);**环只属于 focus** —— invalid 聚焦时长出的环是 danger,且不被 accent 抢走。**聚焦总色量恒 2px**:常态的发丝线不参与色量,环出满 2px;错误态的 danger 边框已出 1px,环只补 1px —— 同色的边框和环视觉上焊成一条,不做这笔账错误态就比常态粗一个 px。
...
- **`hn-field`** —— 输入面:fill 经 `--hn-field-bg` 注入(primary = surface,secondary = inset),海拔阴影经 `--hn-field-shadow` 注入(primary = `--hn-shadow-sm`,secondary 无);hover 在填充里落墨一档、focus 墨退净 + 环从边缘长出(box-shadow 首槽 spread 0→2px,颜色为每状态常量:常态 accent、`[data-invalid]` danger;发丝线边框永不变色);全通道 `fast` 双向 `enter` 单拍。invalid 静止是常规粗细 danger 线 + 淡墨,聚焦才长环且 danger 优先,环只补 1px(边框已出 1px,聚焦总色量与常态对齐在 2px)。caret 保持浏览器默认(宽度与颜色均不做文章)。表单输入件都挂它。
...
出场一律 `exit` 200ms + `ease-exit`,由 utility 自己带上,调用方不需要也不能单独指定。
