<script setup lang="ts">
  import { BookOpen, Code, ArrowUpRight } from '@lucide/vue'
  import {
    NavigationMenu,
    NavigationMenuItem,
...
<template>
  <Stack gap="lg" align="start" class="min-h-80 w-full">
    <NavigationMenu
      v-for="size in ['sm', 'md', 'lg'] as const"
      :key="size"
      :label="`${size} navigation`"
      :size="size"
    >
      ...
      <NavigationMenuItem>
        <NavigationMenuLink href="#usage" active>Usage</NavigationMenuLink>
      </NavigationMenuItem>
      <NavigationMenuItem>
