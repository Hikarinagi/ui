...
<template>
  <Stack class="w-full max-w-2xl" data-demo-carousel-grouped>
    <Inline gap="lg" align="end">
      <FormField label="Container width" class="min-w-40 flex-1">
        <Slider v-model="width" :min="50" :max="100" :format="value => value + '%'" />
      </FormField>
      <Switch v-model="rtl" class="mb-1">Right-to-left</Switch>
    </Inline>
    <Text size="sm" tone="muted">
      Portrait, square and landscape covers keep their proportions. Resize the container to regroup
      them.
    </Text>
    <Carousel
      :items="items"
    ...
    >
      ...
      <template #controls="{ prev, next, canPrev, canNext, index, snapCount }">
        <Inline justify="between" class="w-full">
          ...
          <Inline gap="xs">
            <IconButton label="Previous group" :disabled="!canPrev" @click="prev">
              <ChevronLeft class="rtl:rotate-180" />
            </IconButton>
            <IconButton label="Next group" :disabled="!canNext" @click="next">
