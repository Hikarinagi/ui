...
`Highlight` 用于标示当前条目。它不进行任何测量：调用方通过 `class` 把它定位在活动条目的范围内，活动条目变化时，它由 Motion 的布局动画从原位置连续平移到新位置，而不是在一处消失、在另一处出现。
...
[Tabs](/components/tabs) 与 [SegmentedControl](/components/segmented-control) 根据 `orientation` 自动选择轴向；[Anchor](/components/anchor) 使用 `y`。
...
| `axis`  | `'x' \| 'y' \| 'both'` | `'both'` | 位移动画的轴向，不影响宽高动画               |
| `as`    | `'div' \| 'li'`        | `'div'`  | 渲染的元素                                   |
