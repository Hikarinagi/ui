...
调宽手柄位于所属表头的可见范围内，并与固定区分隔线分开。横向滚动或调宽后，部分被固定列遮挡的表头仍保留可操作的手柄；完全离开可见区的手柄不参与点击和键盘导航。悬停通过 [Tooltip](/components/tooltip) 显示列名，拖动时显示列名和当前像素宽度。交互调整固定列宽度时，为中间非固定列保留至少 48px 的可视区域。
