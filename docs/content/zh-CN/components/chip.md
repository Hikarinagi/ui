...
条目有两种用法：`selectable` 让它可以选中，`removable` 让它带一个移除按钮，两者只能取其一。不需要交互的短标注用 `Tag`。
...
设置 `selectable` 后条目渲染为按钮，`v-model:selected` 绑定选中状态。选中的条目着强调色，并在文字前显示勾选图标。

<Demo name="chip/selectable" />

### 可移除 {#removable}

设置 `removable` 后条目末尾有一个移除按钮，点击时触发 `remove` 事件，条目本身不可点击。
...
图标放入 `icon` 插槽。可选中的条目选中后，图标位切换为勾选图标。
...
- 可选中的条目是原生按钮，点击、回车键与空格键都能切换。
- 选中后文字前出现勾选图标；带 `icon` 插槽时，图标切换为勾选图标。
...
- 可选中的条目带 `aria-pressed`，屏幕阅读器按切换按钮朗读。
